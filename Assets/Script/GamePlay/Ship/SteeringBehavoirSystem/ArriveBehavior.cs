...
public class ArriveBehavior : SteeringBehavior
{
    ...
    [BurstCompile]
    public struct ArriveBehaviorJobs_OneOnOne : IJobParallelForBatch
    {

        [ReadOnly] public NativeArray<BoidJobData> job_boidData;
        [ReadOnly] public NativeArray<SteeringControllerJobData> job_steeringControllerData;
        [ReadOnly] public NativeArray<BoidJobData> job_arriveTargetBoidData;
        ...
        public void Execute(int startIndex, int count)
        {
            for (int i = startIndex; i < startIndex + count; i++)
            {
                direction = job_arriveTargetBoidData[i].position - job_boidData[i].position;
                distance = math.length(direction);
